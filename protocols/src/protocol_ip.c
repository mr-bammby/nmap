...
int16_t protocol_ip_header_init(uint8_t *buffer, uint8_t buffer_len, const protocol_ip_header_t *header)
{
    ...
    {
        return PROTOCOL_IP_ERR_INVALID_ARGUMENT; // Invalid argument
    }

    if (buffer_len < PROTOCOL_IP_MIN_HEADER_LEN)
    {
        return PROTOCOL_IP_ERR_BUFFER_TOO_SMALL; // Buffer too small for minimum IP header
    }
    ...
    *ip_hdr_version_length = (PROTOCOL_IP_VERSION << 4) | (PROTOCOL_IP_MIN_HEADER_LEN/4); // IPv4 and header length
    *ip_hdr_type_of_service = PROTOCOL_IP_DEFAULT_TOS; // Default TOS
    *ip_hdr_total_length = 0; // Temporarily zero, will be covered by encapsulate function
    *ip_hdr_id = htons(header->id); // Identification
    *ip_hdr_offset = htons(PROTOCOL_IP_DEFAULT_OFFSET); // Offset/fragmentation flags (Don't Fragment)
...
int16_t protocol_ip_header_encapsulate(uint8_t *buffer, uint16_t payload_length)
{
    ...
    uint16_t total_length = PROTOCOL_IP_MIN_HEADER_LEN + payload_length;
    *ip_hdr_total_length = htons(total_length);
    *ip_hdr_checksum = 0; // Reset checksum before calculation
    uint16_t calc_checksum = protocol_utils_checksum_final(buffer, PROTOCOL_IP_MIN_HEADER_LEN, 0);
...
int16_t protocol_ip_header_parse(const uint8_t *buffer, uint8_t buffer_len, protocol_ip_header_t *header)
{
    ...
    if (buffer_len < PROTOCOL_IP_MIN_HEADER_LEN)
    {
        return PROTOCOL_IP_ERR_BUFFER_TOO_SMALL;
    }
    ...
    if (ihl < PROTOCOL_IP_MIN_HEADER_LEN || buffer_len < ihl)
    {
        return PROTOCOL_IP_ERR_BUFFER_TOO_SMALL;
    }
